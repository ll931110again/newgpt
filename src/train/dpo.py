...
def main() -> None:
    ...
    if not model_path:
        raise SystemExit("base_model.model_name_or_path is required for DPO")
    ...
    training_args = TrainingArguments(
        ...
        run_name=str(cfg.get("run_name", "dpo")),
    )

    beta = float(deep_get(cfg, "dpo", "beta", default=0.1))

    # Expected columns: prompt, chosen, rejected (or compatible mapping)
    trainer = DPOTrainer(
        model=model_path,
        ref_model=None,
        args=training_args,
        ...
        tokenizer=tokenizer,
        max_length=seq_len,
        max_prompt_length=min(1024, seq_len // 2),
    )
